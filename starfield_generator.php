...
function drawStar($img, $x, $y, $r, $g, $b, $radius)
{
	...
	for ($cx=$minX;$cx <= $maxX;++$cx)
	{
		for ($cy=$minY;$cy < $maxY;++$cy)
		{
			...
			imagesetpixel($img, $cx, $cy, imagecolorallocate($img, min($er + ($r * $gradient), 255), min($eg + ($g * $gradient), 255), min($eb + ($b * $gradient), 255)));
		}
	}
}
...
imagepng($img, 'out_' . preg_replace('/[^A-Za-z0-9]/', '_', date(DATE_W3C)) . '.png');